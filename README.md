...
- **章节 / 区间摘要**：每章一份摘要，每 10 章一份区间摘要，点击即建，边读边回顾；摘要跟不上正文改动时标 ⚠ 提醒维护。
...
扩展注册了 13 个结构化工具：读类（获取当前上下文、列书 / 卷 / 章 / 条目、读章节与区间摘要、设置进度）与写类（书籍 / 分卷 / 章节 / 章节版本 / 条目 / 分类的新建、重命名、移动、插入与删除）。条目与分类用 `kind` 参数区分世界书 / 角色卡 / 笔记，写操作再用 `action` 参数区分具体动作，因此同类能力只需一个工具。章节正文的读写搜索直接用内置文件工具即可。所有写操作自动落在 Git 快照中，放心交给 Copilot。
