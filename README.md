...
- **子书架**：在书架视图中自建分组，书以链接形式收录（不复制），改名 / 删除自动级联同步。
...
- **条目分类**：世界书、角色卡、笔记三处一致——子目录即分类，支持多级嵌套（如 `地理/城邦`），视图内折叠展开；分类可新建子分类、重命名、删除，条目可归入任意层级，也可直接放在根目录。
...
- **与 Copilot 共创**：注册 40 个语言模型工具，Copilot 可直接读写书库。
...
扩展注册了 40 个结构化工具（列书 / 列卷 / 列章、读摘要、设置进度、新建 / 移动 / 重命名 / 删除卷章书与条目、世界书/角色卡/笔记的分类管理），章节正文的读写搜索直接用内置文件工具即可。所有写操作自动落在 Git 快照中，放心交给 Copilot。
