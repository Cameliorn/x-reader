...
async function requireChapter(library: LibraryService, book: BookInfo, ref: string): Promise<ChapterFile> {
	...
	if (!chapter) {
		throw new Error(
			vscode.l10n.t('Chapter “{0}” not found. Use a relative path, file name, or title to reference a chapter.', ref)
		);
	}
	return chapter;
}
...
/** 读章节摘要：文件缺失时不报错，返回「尚未创建 + 应写入的路径」，让 agent 用文件工具写。 */
async function readChapterSummary(
	library: LibraryService,
	book: BookInfo,
	ref: string
): Promise<vscode.LanguageModelToolResult> {
	const chapter = await requireChapter(library, book, ref);
...
async function readIntervalSummary(
	...
	const target = intervals.find((i) => i.fileName === range || (i.startSeq === nums[0] && i.endSeq === nums[1]));
	if (!target) {
		throw new Error(
			vscode.l10n.t(
...
export function registerAgentTools(context: vscode.ExtensionContext, library: LibraryService): void {
	...
	context.subscriptions.push(
		...
		vscode.lm.registerTool<BookInput & { scope: string; chapter?: string; range?: string }>(
			'xReader_readSummary',
			{
				async invoke(options) {
					const scope = requireEnum(options.input.scope, ['chapter', 'interval'] as const, 'scope');
					const book = await resolveBook(library, options.input.book);
					return scope === 'chapter'
						? readChapterSummary(library, book, requireParam(options.input.chapter, 'chapter'))
						: readIntervalSummary(library, book, options.input.range?.trim());
				},
		...
		vscode.lm.registerTool<ManageChapterInput>('xReader_manageChapter', {
			async invoke(options) {
				...
				const action = requireEnum(
					input.action,
					['create', 'insert', 'rename', 'move', 'delete'] as const,
					'action'
				);
				const book = await resolveBook(library, input.book);
				switch (action) {
					...
					case 'insert': {
						...
						const anchor = await requireChapter(library, book, (after ?? before) as string);
